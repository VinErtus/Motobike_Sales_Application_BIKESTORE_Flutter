...
class CategoryBuilders extends StatefulWidget {
  const CategoryBuilders({Key? key}) : super(key: key);

  @override
  State<CategoryBuilders> createState() => _CategoryBuilderState();
}

class _CategoryBuilderState extends State<CategoryBuilders> {
    ...
    return Dismissible(
      ...
      onDismissed: (direction) async {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('${breed.name} deleted'),
            duration: const Duration(seconds: 2),
          ),
        );
      },
      background: Container(
        color: const Color.fromARGB(255, 249, 177, 64), // Background color for swipe action
        child: const Align(
