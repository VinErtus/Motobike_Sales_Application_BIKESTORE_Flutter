...
class DatabaseHelper {
  ...
  Future<void> insertProductFav(Fav productModel) async {
  ...
  Future<List<Fav>> productsFav() async {
    final db = await database;
    final List<Map<String, dynamic>> maps = await db.query('Fav');
    return List.generate(
      maps.length,
          (index) => Fav.fromMap(maps[index]),
    );
  }
  Future<void> deleteProductFav(int id) async {
    final db = await database;
    await db.delete(
      'Fav',
      where: 'productID = ?',
  ...
  Future<Fav> productFav(int id) async {
    final db = await database;
    final List<Map<String, dynamic>> maps =
    await db.query('Fav', where: 'productID = ?', whereArgs: [id]);
    if (maps.isEmpty) throw Exception('Product not found');
    return Fav.fromMap(maps.first);
  }
